      ...
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.map((testimonial, i) => (
            <div
              key={i}
              className="bg-card/60 backdrop-blur-sm rounded-2xl p-8 border border-border/50 hover:border-primary/30 transition-all hover:shadow-lg hover:shadow-primary/5"
            >
          ...
          <div className="inline-flex items-center gap-2 bg-primary/10 px-6 py-3 rounded-full border border-primary/20">
            <div className="flex -space-x-2">
              {["👨‍💼", "👩‍💻", "👨‍🔧", "👩‍🎨"].map((emoji, i) => (
                <div
                  key={i}
                  className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center border-2 border-background text-sm"
                >
                  {emoji}
                </div>
              ))}
